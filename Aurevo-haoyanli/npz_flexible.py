...
CONFIG = {
    ...
    # RANSAC参数
    "RANSAC_DISTANCE_THRESHOLD": 0.02,  # 2cm
...
def depth_to_point_cloud(depth_mm):
    """将深度图转换为3D点云"""
    h, w = depth_mm.shape
    ...
    # 有效深度掩码
    valid_mask = (depth_mm > CONFIG["MIN_DEPTH_MM"]) & (depth_mm < CONFIG["MAX_DEPTH_MM"])
    
    # 转换为相机坐标系
    z = depth_mm[valid_mask] / 1000.0  # 转换为米
    x = (xx[valid_mask] - CONFIG["CAMERA_CX"]) * z / CONFIG["CAMERA_FX"]
    y = (yy[valid_mask] - CONFIG["CAMERA_CY"]) * z / CONFIG["CAMERA_FY"]
...
def batch_process_multiple_files():
    """批量处理多个文件找出最佳平面"""
    depth_dir = Path(CONFIG["NPZ_DIRECTORY"])
    ...
    for filename in CONFIG["BATCH_FILES"]:
        ...
        if npz_path.exists():
            ...
            try:
                ...
                if result and result['angle'] < 45:  # 只保留相对水平的平面
                    results.append({
                        'filename': filename,
                        'plane': result['plane'],
                        'angle': result['angle'],
    ...
    if results:
        ...
        print(f"\n{'='*60}")
        print(f"最佳桌面平面 (文件 {best['filename']}):")
        plane = best['plane']
...
def main():
    ...
    print("="*60)
    print(f"配置信息:")
    print(f"  NPZ目录: {CONFIG['NPZ_DIRECTORY']}")
    print(f"  测试文件: {CONFIG['TEST_FILE']}")
    print(f"  深度范围: {CONFIG['MIN_DEPTH_MM']}-{CONFIG['MAX_DEPTH_MM']}mm")
    
    # 设置文件路径
    depth_dir = Path(CONFIG["NPZ_DIRECTORY"])
    test_file = depth_dir / CONFIG["TEST_FILE"]
